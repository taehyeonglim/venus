...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({ result, image, onRestart, apiKey }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-8">
      ...
      <div className="relative">
        ...
        <div className="relative venus-card rounded-3xl overflow-hidden">
          ...
          <div className="relative p-8 md:p-10 text-center">
            ...
            <div className="absolute top-0 left-1/4 w-32 h-32 bg-[#FFB7C5]/10 rounded-full blur-3xl"></div>
            <div className="absolute top-0 right-1/4 w-32 h-32 bg-[#FFB7C5]/10 rounded-full blur-3xl"></div>
            ...
            <div className="relative inline-block mb-6">
              ...
              <div className="relative">
                <img
                  src={image}
                  alt="Analyzed face"
                  className="w-28 h-28 md:w-32 md:h-32 rounded-full object-cover border-2 border-[#FFB7C5]/50"
                />
                {/* Badge */}
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#9FE2BF]/80 font-medium">{getScoreLabel(result.overallScore)}</p>
            </div>
          </div>

          {/* Stats Section */}
          <div className="grid grid-cols-1 md:grid-cols-2 border-t border-[#FFB7C5]/10">
            {/* Radar Chart */}
            <div className="p-6 h-[280px]">
              <h3 className="text-xs font-medium text-[#9FE2BF]/60 uppercase tracking-widest mb-4">
                분석 레이더
              </h3>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="75%" data={radarData}>
                  <PolarGrid stroke="rgba(159, 226, 191, 0.2)" />
                  <PolarAngleAxis
                    dataKey="subject"
                    tick={{ fill: 'rgba(159, 226, 191, 0.7)', fontSize: 12 }}
                  />
                  <defs>
                    <linearGradient id="radarGradient" x1="0" y1="0" x2="1" y2="1">
                      <stop offset="0%" stopColor="#FFB7C5" stopOpacity={0.8} />
                      <stop offset="100%" stopColor="#9FE2BF" stopOpacity={0.8} />
                    </linearGradient>
                  </defs>
                  <Radar
                    name="Score"
            ...
            {/* Celebrity & Features */}
            <div className="p-6 flex flex-col justify-center gap-6 bg-[#FFB7C5]/5">
              <div>
                <h3 className="text-xs font-medium text-[#9FE2BF]/60 uppercase tracking-widest mb-2">
                  닮은 꼴 연예인
                </h3>
              ...
              <div>
                <h3 className="text-xs font-medium text-[#9FE2BF]/60 uppercase tracking-widest mb-3">
                  최고의 강점
                </h3>
                <div className="flex flex-wrap gap-2">
                  {result.bestFeatures.map((feature, i) => (
                    <span
                      key={i}
                      className="px-3 py-1.5 rounded-full text-sm font-medium bg-[#FFB7C5]/10 text-[#9FE2BF] border border-[#FFB7C5]/20"
                    >
                      {feature}
                    </span>
                  ))}
          ...
          {/* Feedback Section */}
          <div className="p-6 md:p-8 space-y-4 border-t border-[#FFB7C5]/10">
            {/* AI Feedback */}
            <div className="venus-card p-5 rounded-2xl">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-medium text-[#9FE2BF]">AI 상세 피드백</h3>
              </div>
              <p className="text-[#9FE2BF]/80 leading-relaxed text-sm italic">
                "{result.feedback}"
              </p>
            </div>

            {/* Style Advice */}
            <div className="relative overflow-hidden venus-card p-5 rounded-2xl border-[#FFB7C5]/20">
              <div className="absolute top-0 right-0 w-24 h-24 bg-[#FFB7C5]/10 rounded-full blur-2xl"></div>
              <div className="relative">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-[#FFB7C5]/20 flex items-center justify-center">
                      <svg className="w-3.5 h-3.5 text-[#FFB7C5]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                      </svg>
                    </div>
                    <h3 className="text-sm font-medium text-[#FFB7C5]">스타일 제안</h3>
                  </div>
                  {/* Alternative Style Button */}
                  <button
                    onClick={handleGetAlternativeStyle}
                    disabled={isLoadingStyle}
                    className="px-3 py-1.5 rounded-lg bg-[#FFB7C5]/10 border border-[#FFB7C5]/20 text-[#9FE2BF] text-xs font-medium hover:bg-[#FFB7C5]/20 transition-all flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isLoadingStyle ? (
                      <>
                ...
                <p className="text-[#9FE2BF]/80 leading-relaxed text-sm mb-4">
                  {currentStyleAdvice}
                </p>
                {styleError && (
                  <p className="text-[#FFB7C5] text-xs mb-3">{styleError}</p>
                )}
                {/* Try Style Button */}
                <button
                  onClick={() => setShowStyleSimulation(true)}
                  className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-[#FFB7C5]/20 to-[#FFB7C5]/20 border border-[#FFB7C5]/30 text-[#FFB7C5] text-sm font-medium hover:from-[#FFB7C5]/30 hover:to-[#FFB7C5]/30 transition-all flex items-center justify-center gap-2"
                >
      ...
      {/* Disclaimer */}
      <p className="text-center text-[#9FE2BF]/40 text-xs leading-relaxed">
        이 결과는 AI의 가상 분석이며 실제 외모의 절대적인 지표가 아닙니다.<br />
        <span className="text-[#9FE2BF]/60 font-medium">당신은 그 자체로 충분히 아름답습니다.</span>
      </p>
    </div>
  );
};
